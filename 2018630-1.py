...
class GA():
    ...
    def result(self):
        ...
        """
        graded = [(self.fitness(chromosome), chromosome) for chromosome in self.population]
        gbest = sorted([self.fitness(chromosome) for chromosome in self.population],reverse=True)[0]
        graded = [x[1] for x in sorted(graded, reverse=True)]
        
        return ga.decode(graded[0]),  gbest
...
if __name__ == '__main__':
    ...
    for x in range(20):
        ga.evolve()
        Result.append(ga.result())

    Fit = [x[1] for x in Result]
    print(Fit)
    plt.figure()
    plt.plot(Fit)
    plt.xticks(range(20))
